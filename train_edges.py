...
opt.input_nc = 1 # Grayscale Edges
...
data_loader = CreateDataLoader(opt) #this will look for an "edge dataset" at the location you specified
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        losses, latest_generated_frame = model(
            left_frame, None,
            right_frame, None,
            infer=(opt.scheduled_sampling or opt.debug)
        )
        if opt.debug:
